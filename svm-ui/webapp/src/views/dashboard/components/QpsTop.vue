<template>
    <div>
        <el-table :data="rows" stripe v-if="config.mode=='service'">
            ...
            <el-table-column prop="value" label="响应时间" width="100"></el-table-column>
        </el-table>
...
<script>
import { queryQpsTop } from "@/api/dashboard";
...
export default {
	...
	methods: {
		refresh(window) {
			queryQpsTop(window, this.config.mode, this.config.count).then(res => {
				this.rows = res;
			});
		}
	}
